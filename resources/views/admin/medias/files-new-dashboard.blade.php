...
<main data-ui="main-file" class="w-full">
    ...
    <nav class="w-full rounded-md bg-neutral-100 px-5 py-3 dark:bg-neutral-600">
        <ol class="list-reset flex">
            ...
            <li>
                <a href="{{ route('dashboard.files.index') }}"
                    class="text-primary transition duration-150 ease-in-out hover:text-primary-600 focus:text-primary-600 active:text-primary-700 dark:text-primary-400 dark:hover:text-primary-500 dark:focus:text-primary-500 dark:active:text-primary-600">Files</a>
            </li>
            <li>
                <span class="mx-2 text-neutral-500 dark:text-neutral-300">/</span>
            </li>
            <li class="break-all overflow-hidden text-ellipsis text-neutral-500 dark:text-neutral-300">
                {{ $file?->full_name }}</li>
        </ol>
    </nav>
    <!-- Bread END -->
    ...
    <h2 class="mt-9 mb-5 px-4 font-bold">
        <div class="flex col-gap-1 items-center">
            ...
            @if($file?->id)
                <submit-changes apiAction="PATCH" apiPoint="{{ route('media.update', ['medium' => $file->id]) }}"
                    event-bus-name="eventBusProjectEdit" itemClass="media" itemId="{{ $file->id }}"
                    scopeselector="[data-ui='main-file']" toincludeidentifier="[mediaData]" />
            @else
                <submit-changes apiAction="POST" apiPoint="{{ route('media.store') }}" event-bus-name="eventBusProjectEdit"
                    itemClass="media" itemId="{{ $file?->id }}" scopeselector="[data-ui='main-file']"
        ...
        <div class="overflow-x-auto ">
            <div class="inline-block min-w-full ">
                <div class="overflow-hidden">
                    <table class="flex flex-col min-w-full text-left text-sm font-light bg-white">
                        <thead class="border-b font-medium dark:border-neutral-500">
                            <tr class="flex">
                                <th scope="col" class="flex-none w-[40px] px-2 py-4">ID</th>
                                <th scope="col" class="flex-1 px-2 py-4 break-all overflow-hidden text-ellipsis">Full
                                    Name</th>
                        ...
                        <tbody class="flex flex-col">
                            <tr class="flex justify-between border-b dark:border-neutral-500">
                                ...
                                <td class="flex-1 px-2 py-4 break-all overflow-hidden text-ellipsis">
                                    {{ $file?->full_name }}</td>
    ...
    <div class="flex flex-col md:flex-row my-8">
        <section class="w-1/1 md:w-3/4">
            ...
            <?php 
            if (!$file?->id): ?>
            <image-drag-drop-upload name="dragDropUploadFiles" event-bus-name="eventBusProjectEdit" mediaData
        ...
        <aside class="w-1/1 md:w-1/4 px-4">
            ...
            <div class="relative mb-3" data-te-input-wrapper-init>
                <textarea
                    ...
                    id="fileDescription" name="fileDescription" mediaData rows="3"
                    placeholder="Short info...">{{ $file?->description }}</textarea>
                <label for="fileDescription"
                    class="pointer-events-none absolute left-3 top-0 mb-0 max-w-[90%] origin-[0_0] truncate pt-[0.37rem] leading-[1.6] text-neutral-500 transition-all duration-200 ease-out peer-focus:-translate-y-[0.9rem] peer-focus:scale-[0.8] peer-focus:text-primary peer-data-[te-input-state-active]:-translate-y-[0.9rem] peer-data-[te-input-state-active]:scale-[0.8] motion-reduce:transition-none dark:text-neutral-200 dark:peer-focus:text-primary">Info
                    on item</label>
            </div>
            <!-- Description END -->
            ...
            <div class="rounded-md border p-1">
                ...
                <div class="relative mb-3" data-te-input-wrapper-init>
                    <input type="text"
                        ...
                        id="fileAlt" name="fileAlt" mediaData placeholder="Alt text" value="" />
                    <!-- disabled -->
                    <label for="fileAlt"
                        class="pointer-events-none absolute left-3 top-0 mb-0 max-w-[90%] origin-[0_0] truncate pt-[0.37rem] text-xs leading-[1.5] text-neutral-500 transition-all duration-200 ease-out peer-focus:-translate-y-[0.75rem] peer-focus:scale-[0.8] peer-focus:text-primary peer-data-[te-input-state-active]:-translate-y-[0.75rem] peer-data-[te-input-state-active]:scale-[0.8] motion-reduce:transition-none dark:text-neutral-200 dark:peer-focus:text-primary">Alt
                    </label>
                </div>
                <!--Alt END -->
